...
#[no_mangle]
pub extern "C" fn CL_ObjectModule_new(module_name: *const c_char) -> *mut ObjectModule {
    ...
    let isa = cranelift_native::builder()
        ...
        .unwrap();
    let name = unsafe { CStr::from_ptr(module_name) }.to_str().unwrap();
    let obj_builder = ObjectBuilder::new(isa, name, cranelift_module::default_libcall_names()).unwrap();
    Box::into_raw(Box::new(ObjectModule::new(obj_builder)))
}

#[no_mangle]
pub extern "C" fn CL_ObjectModule_declare_function(
    ...
    m.declare_function(fname, Linkage::Export, s).unwrap().as_u32()
}

#[no_mangle]
pub extern "C" fn CL_ObjectModule_define_function(
    module: *mut ObjectModule,
    func_id: u32,
    func: *mut Function,
) -> () {
    let m = unsafe { &mut *module };
    let f = unsafe { Box::from_raw(func) };
    let mut ctx = Context::for_function(*f);
    m.define_function(cranelift_module::FuncId::from_u32(func_id), &mut ctx).unwrap();
}

#[no_mangle]
pub extern "C" fn CL_ObjectModule_finish_and_emit(
    module: *mut ObjectModule,
    path: *const c_char,
) -> i32 {
    let m = unsafe { Box::from_raw(module) };
